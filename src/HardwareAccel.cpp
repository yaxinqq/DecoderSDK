...
HardwareAccel::HardwareAccel()
    : type_(HWAccelType::NONE), hwDeviceCtx_(nullptr), hwPixFmt_(AV_PIX_FMT_NONE), initialized_(false), deviceIndex_(0)
{
}
...
bool HardwareAccel::init(HWAccelType type, int deviceIndex)
{
    ...
    // 如果类型为NONE，直接返回成功
    if (type_ == HWAccelType::NONE) {
        return true;
    }

    // 确定硬件设备类型
    AVHWDeviceType deviceType;
    if (type_ == HWAccelType::AUTO) {
        deviceType = findBestHWAccelType();
        if (deviceType == AV_HWDEVICE_TYPE_NONE)
        {
            std::cerr << "No suitable hardware acceleration method found" << std::endl;
            return false;
        }
    ...
    {
        std::cerr << "Failed to initialize hardware device: " << getHWAccelTypeName(type_) << std::endl;
        return false;
    }
    ...
    {
        std::cerr << "Failed to get hardware pixel format for device: " << getHWAccelTypeName(type_) << std::endl;
        av_buffer_unref(&hwDeviceCtx_);
        hwDeviceCtx_ = nullptr;
        return false;
    }
...
AVFrame *HardwareAccel::getHWFrame(AVFrame *frame)
{
    ...
    if (ret < 0)
    {
        ...
        std::cerr << "Failed to allocate hardware frame buffer: " << errBuf << std::endl;
    ...
    if (ret < 0)
    {
        char errBuf[AV_ERROR_MAX_STRING_SIZE];
        av_strerror(ret, errBuf, sizeof(errBuf));
        std::cerr << "Failed to transfer frame data to hardware: " << errBuf << std::endl;
        av_frame_free(&hwFrame);
        return nullptr;
    }
...
bool HardwareAccel::transferFrameToHost(AVFrame *hwFrame, AVFrame *swFrame)
{
    ...
    {
        std::cerr << "Not a hardware frame" << std::endl;
        return false;
    }
    ...
    if (ret < 0)
    {
        char errBuf[AV_ERROR_MAX_STRING_SIZE];
        av_strerror(ret, errBuf, sizeof(errBuf));
        std::cerr << "Failed to transfer frame data to host: " << errBuf << std::endl;
        return false;
    }
...
std::vector<HWAccelInfo> HardwareAccel::getSupportedHWAccelTypes()
{
    ...
    while ((type = av_hwdevice_iterate_types(type)) != AV_HWDEVICE_TYPE_NONE)
    {
        ...
        {
            // 获取硬件像素格式
            info.hwFormat = getHWPixelFormatForDevice(type);
            ...
            if (info.hwFormat != AV_PIX_FMT_NONE)
            {
                ...
                if (hwFramesCtx)
                {
                    ...
                    if (ret >= 0)
                    {
                        ...
                        if (constraints)
                        {
                            for (const AVPixelFormat *p = constraints->valid_sw_formats; p && *p != AV_PIX_FMT_NONE; p++)
                            {
...
HWAccelType HardwareAccel::fromAVHWDeviceType(AVHWDeviceType avType)
{
    ...
    case AV_HWDEVICE_TYPE_DXVA2:
        return HWAccelType::DXVA2;
    case AV_HWDEVICE_TYPE_D3D11VA:
        return HWAccelType::D3D11VA;
...
AVPixelFormat HardwareAccel::getHWPixelFormat(AVCodecContext *codecCtx, const AVPixelFormat *pix_fmts)
{
    // 查找与解码器上下文关联的硬件加速对象
...
std::shared_ptr<HardwareAccel> HardwareAccelFactory::createHardwareAccel(HWAccelType type, int deviceIndex)
{
