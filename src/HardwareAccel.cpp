...
bool HardwareAccel::init(HWAccelType type, int deviceIndex)
{
    ...
    if (!initHWDevice(deviceType, deviceIndex_)) {
        LOG_WARN("Failed to initialize hardware device: {}",
                 getHWAccelTypeName(type_));
        return false;
    }
    ...
    if (hwPixFmt_ == AV_PIX_FMT_NONE) {
        LOG_WARN("Failed to get hardware pixel format for device: {}",
                 getHWAccelTypeName(type_));
        av_buffer_unref(&hwDeviceCtx_);
        hwDeviceCtx_ = nullptr;
        return false;
...
bool HardwareAccel::initHWDevice(AVHWDeviceType deviceType, int deviceIndex)
{
    ...
    // 创建硬件设备上下文
    int ret = av_hwdevice_ctx_create(&hwDeviceCtx_, deviceType, deviceName,
                                     nullptr, 0);
    if (ret < 0) {
        char errBuf[AV_ERROR_MAX_STRING_SIZE];
        av_strerror(ret, errBuf, sizeof(errBuf));
...
AVPixelFormat HardwareAccel::getHWPixelFormatForDevice(
    AVHWDeviceType deviceType)
{
    switch (deviceType) {
        case AV_HWDEVICE_TYPE_DXVA2:
...
std::shared_ptr<HardwareAccel> HardwareAccelFactory::createHardwareAccel(
    HWAccelType type, int deviceIndex)
{
