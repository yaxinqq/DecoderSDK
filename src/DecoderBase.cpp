...
DecoderBase::DecoderBase(std::shared_ptr<Demuxer> demuxer,
    ...
    : demuxer_(demuxer),
      ...
      frameQueue_(kFrameQueueDefaultSize, false),
      isRunning_(false)
{
    statistics_.reset();
}
...
double DecoderBase::calculatePts(const Frame &frame) const
{
    ...
    const int64_t pts = (frame.avPts() != AV_NOPTS_VALUE)
                            ? frame.avPts()
...
bool DecoderBase::handleFirstFrame(const std::string &decoderName,
                                   MediaType mediaType,
                                   const std::string &description)
{
    auto event = std::make_shared<DecoderEventArgs>(
        decoderName, streamIndex_, mediaType,
        codecCtx_->hw_device_ctx != nullptr, decoderName, description);
    eventDispatcher_->triggerEvent(EventType::kDecodeFirstFrame, event);

    return true;
}

bool DecoderBase::handleDecodeError(const std::string &decoderName,
                                    MediaType mediaType, int errorCode,
                                    const std::string &description)
{
...
bool DecoderBase::handleDecodeRecovery(const std::string &decoderName,
                                       MediaType mediaType,
                                       const std::string &description)
{
    auto event = std::make_shared<DecoderEventArgs>(
    ...
    std::optional<std::chrono::steady_clock::time_point> &lastFrameTime)
{
    ...
    // 更新上一帧时间
    lastFrameTime = currentTime + std::chrono::microseconds(
                                      static_cast<int64_t>(baseDelay * 1000.0));

    return std::max(0.0, baseDelay);
}

bool DecoderBase::checkAndUpdateSerial(int &currentSerial,
