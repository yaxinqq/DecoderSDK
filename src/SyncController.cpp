...
void SyncController::setMaster(MasterClock m)
{
...
void SyncController::setAVSyncThreshold(double threshold)
{
    syncThreshold_.store(
        std::clamp(threshold, kMinSyncThreshold, kMaxSyncThreshold),
...
double SyncController::getMasterClockCached() const
{
    double masterClock;
    ...
    masterClockCacheTime_.store(av_gettime_relative(),
                                std::memory_order_release);
...
// EMA 平滑函数（改进版）
double smoothEMA(double alpha, double prev, double current,
                                         ...
                                         double baseDelay, double speed)
{
    ...
    // 自适应阈值
    double threshold = adaptiveSync_.load() ? computeAdaptiveThreshold()
                                            : syncThreshold_.load();
    threshold /= speed;
    ...
    if (newDrift < -threshold) {
        ...
        if (shouldDropFrame(framePts, frameDuration)) {
            ...
            LOG_DEBUG("Dropping frame, drift: {:.3f}ms, threshold: {:.3f}ms",
                      newDrift * 1000, threshold * 1000);
            return -1.0;
        }
    }
    ...
    if (std::abs(newDrift) > threshold) {
        ...
        // 检查是否需要重复帧
        if (newDrift > threshold &&
            shouldDuplicateFrame(framePts, frameDuration)) {
                                         ...
                                         double speed)
{
    ...
    double newDrift =
        smoothEMA(alpha_.load(), prevDrift, diff, 0.05);  // 音频变化更保守
...
bool SyncController::shouldDropFrame(double framePts,
...
bool SyncController::shouldDuplicateFrame(double framePts,
                                          double frameDuration) const
{
    double drift = smoothedVideoDrift_.load(std::memory_order_acquire);
    double threshold = syncThreshold_.load();
...
SyncController::SyncState SyncController::getSyncState() const
{
    double videoDrift =
        std::abs(smoothedVideoDrift_.load(std::memory_order_acquire));
    double audioDrift =
        std::abs(smoothedAudioDrift_.load(std::memory_order_acquire));
...
void SyncController::updateSyncParameters()
{
...
SyncController::SyncQualityStats SyncController::getSyncQualityStats() const
{
...
// 私有方法实现
void SyncController::updateSyncQuality(double drift)
{
    // 递增总的同步质量计数器
    int currentCount =
        syncQualityCounter_.fetch_add(1, std::memory_order_acq_rel) + 1;
    ...
    while (drift > currentMax &&
           !maxDrift_.compare_exchange_weak(currentMax, drift,
                                            std::memory_order_release,
    ...
    if (drift <= threshold) {
        ...
        if (drift > threshold * 3.0) {
            LOG_WARN(
                "Severe sync drift detected: {:.3f}ms (threshold: {:.3f}ms)",
                drift * 1000, threshold * 1000);
        }
    }
    ...
    if (currentCount % 500 == 0) {
        double goodRate =
            static_cast<double>(goodSyncCount_.load()) / currentCount * 100.0;
        LOG_INFO(
...
double SyncController::computeAdaptiveThreshold() const
{
    double videoDrift =
        std::abs(smoothedVideoDrift_.load(std::memory_order_acquire));
    double audioDrift =
...
SyncController::SyncState SyncController::evaluateSyncState(double drift) const
{
    double threshold = syncThreshold_.load(std::memory_order_acquire);
