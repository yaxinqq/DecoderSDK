...
namespace {
    std::string getTimestampedLogFile(const std::string& dir) {
        ...
        oss << dir << "/" << std::put_time(&tm, "%Y%m%d%H%M%S") << ".log";
        return oss.str();
    }
    ...
    spdlog::level::level_enum parseLevel(const std::string& level) {
        ...
        if (l == "warn") return spdlog::level::warn;
    ...
    bool loadLogConfigFromJson(const std::string& path, LogConfig& config) {
        ...
        if (!in.is_open()) {
            std::cerr << "未找到日志配置文件: " << path << std::endl;
            return false;
        }
        ...
        try {
            ...
            if (j.contains("log")) {
                auto log = j["log"];
                if (log.contains("enableFileLog")) config.enableFileLog = log["enableFileLog"];
                if (log.contains("level")) config.level = log["level"];
                if (log.contains("logDir")) config.logDir = log["logDir"];
                if (log.contains("maxFileSizeMB")) config.maxFileSizeMB = log["maxFileSizeMB"];
                if (log.contains("maxFiles")) config.maxFiles = log["maxFiles"];
            }
            return true;
