...
Demuxer::Demuxer(std::shared_ptr<EventDispatcher> eventDispatcher)
    : eventDispatcher_(eventDispatcher),
      mediaRecorder_(std::make_unique<MediaRecorder>(eventDispatcher))
{
}
...
bool Demuxer::open(const std::string &url, bool isRealTime, bool isReopen)
{
    ...
    // 上报流正在打开事件
    auto event =
        std::make_shared<StreamEventArgs>(url, "Demuxer", "Stream Opening");
    ...
    // 打开输入
    int ret =
        avformat_open_input(&formatContext_, url.c_str(), nullptr, &options);
...
bool Demuxer::seek(double position)
{
    ...
    // 执行seek
    int ret =
        av_seek_frame(formatContext_, -1, timestamp, AVSEEK_FLAG_BACKWARD);
    if (ret < 0) {
...
bool Demuxer::startRecording(const std::string &outputPath)
{
    ...
    return mediaRecorder_->startRecording(outputPath, formatContext_);
}

bool Demuxer::stopRecording()
{
    return mediaRecorder_->stopRecording();
}
        ...
        if (ret < 0) {
            if (ret == AVERROR_EOF || avio_feof(formatContext_->pb)) {
                ...
                if (++errorCount >= kReadErrorMaxCount) {
                    LOG_ERROR("Too many read errors ({}) in {}, stopping", url_,
                              errorCount);
...
void Demuxer::distributePacket(AVPacket *pkt)
{
    ...
    if (pkt->stream_index == videoStreamIndex_ && videoPacketQueue_) {
        packet.setSerial(videoPacketQueue_->serial());
        videoPacketQueue_->push(packet, -1);

        // 如果正在录制，写入录制器
        if (mediaRecorder_->isRecording()) {
            mediaRecorder_->writePacket(packet, AVMEDIA_TYPE_VIDEO);
        }
    } else if (pkt->stream_index == audioStreamIndex_ && audioPacketQueue_) {
        packet.setSerial(audioPacketQueue_->serial());
        audioPacketQueue_->push(packet, -1);

        // 如果正在录制，写入录制器
        if (mediaRecorder_->isRecording()) {
            mediaRecorder_->writePacket(packet, AVMEDIA_TYPE_AUDIO);
        }
    }
}
