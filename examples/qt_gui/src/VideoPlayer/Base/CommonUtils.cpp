...
void registerVideoMetaType()
{
    ...
    qRegisterMetaType<decoder_sdk::LoopMode>("decoder_sdk::LoopMode");
    ...
    qRegisterMetaType<Stream::ClockSourceType>("Stream::ClockSourceType");
    qRegisterMetaType<Stream::PlaybackSyncOptions>("Stream::PlaybackSyncOptions");

    qRegisterMetaType<std::weak_ptr<DisplayRenderer>>("std::weak_ptr<DisplayRenderer>");
}
...
namespace cuda_utils {
    ...
    typedef CUresult(CUDAAPI *PFN_cuDevicePrimaryCtxGetState)(CUdevice dev, unsigned int *flags, int *active);
    ...
    typedef CUresult(CUDAAPI *PFN_cuStreamAddCallback)(CUstream hStream, CUstreamCallback callback, void *userData, unsigned int flags);
    typedef CUresult(CUDAAPI *PFN_cuGraphicsGLRegisterImage)(CUgraphicsResource *pCudaResource, unsigned int image, unsigned int target, unsigned int Flags);
    typedef CUresult(CUDAAPI *PFN_cuGraphicsUnregisterResource)(CUgraphicsResource resource);
    typedef CUresult(CUDAAPI *PFN_cuGraphicsMapResources)(unsigned int count, CUgraphicsResource *resources, CUstream hStream);
    typedef CUresult(CUDAAPI *PFN_cuGraphicsUnmapResources)(unsigned int count, CUgraphicsResource *resources, CUstream hStream);
    typedef CUresult(CUDAAPI *PFN_cuGraphicsSubResourceGetMappedArray)(CUarray *pArray, CUgraphicsResource resource, unsigned int arrayIndex, unsigned int mipLevel);
...
#define ck(call) check(call, __LINE__, __FILE__)
    ...
    public:
        ...
        CUcontext getContext()
        {
            ...
            if (deviceActived && deviceFlags != desiredFlags) {
                qWarning() << QStringLiteral("CUDA Primary context already active with incompatible flags!");
    ...
    private:
        ...
        void initialize()
        {
            ...
            if (deviceIndex < 0 || deviceIndex >= deviceCount) {
                qInfo() << QStringLiteral("Invalid CUDA device index: %1, use 0 instead!").arg(deviceIndex);
                deviceIndex = 0;
            }
    ...
    {
        return g_cudaLibLoaded ? g_cudaFuncs.cuStreamAddCallback(hStream, callback, userData, flags) : CUDA_ERROR_UNKNOWN;
    }
    ...
    {
        return g_cudaLibLoaded ? g_cudaFuncs.cuGraphicsGLRegisterImage(pCudaResource, image, target, Flags) : CUDA_ERROR_UNKNOWN;
    }

    CUresult cuGraphicsUnregisterResource(CUgraphicsResource resource)
    {
        return g_cudaLibLoaded ? g_cudaFuncs.cuGraphicsUnregisterResource(resource) : CUDA_ERROR_UNKNOWN;
    }

    CUresult cuGraphicsMapResources(unsigned int count, CUgraphicsResource *resources, CUstream hStream)
    {
        return g_cudaLibLoaded ? g_cudaFuncs.cuGraphicsMapResources(count, resources, hStream) : CUDA_ERROR_UNKNOWN;
    }

    CUresult cuGraphicsUnmapResources(unsigned int count, CUgraphicsResource *resources, CUstream hStream)
    {
        return g_cudaLibLoaded ? g_cudaFuncs.cuGraphicsUnmapResources(count, resources, hStream) : CUDA_ERROR_UNKNOWN;
    }

    CUresult cuGraphicsSubResourceGetMappedArray(CUarray *pArray, CUgraphicsResource resource, unsigned int arrayIndex, unsigned int mipLevel)
    {
        return g_cudaLibLoaded ? g_cudaFuncs.cuGraphicsSubResourceGetMappedArray(pArray, resource, arrayIndex, mipLevel) : CUDA_ERROR_UNKNOWN;
    }
    ...
    private:
        ...
        void initialize()
        {
            ...
            for (UINT i = 0; i < adapterCount; ++i) {
                ...
                if (glRenderer.contains(adapterDesc, Qt::CaseInsensitive)) {
                    adapterIndex = i;
                    qInfo() << QStringLiteral("Find adapter index: %1").arg(QString::number(adapterIndex));
                    break; // 使用OpenGL Context对应的设备
                }
            }
...
#undef FORCE_INLINE
    ...
    public:
        ...
        void cleanup()
        {
            ...
            try {
                ...
                qWarning() << QStringLiteral("Destroy VkDevice Failed: %1").arg(QString::fromStdString(e.what()));
            }
            ...
            try {
                ...
                qWarning() << QStringLiteral("Destroy VkInstance Failed: %1").arg(QString::fromStdString(e.what()));
            }
    ...
    void VulkanManager::initialize()
    {
        try {
            ...
            qInfo() << QStringLiteral("Physical device selected, device name: %1").arg(QString::fromStdString(vkPhysicalDevice_.name));
    ...
    bool VulkanManager::selectPhysicalDevice()
    {
        ...
        if (!vkPhysicalDevice_.enable_extension_if_present(VK_KHR_VIDEO_DECODE_AV1_EXTENSION_NAME)) {
            qWarning() << QStringLiteral("Not supported %1").arg(VK_KHR_VIDEO_DECODE_AV1_EXTENSION_NAME);
        }
        if (!vkPhysicalDevice_.enable_extension_if_present(VK_KHR_VIDEO_DECODE_VP9_EXTENSION_NAME)) {
            qWarning() << QStringLiteral("Not supported %1").arg(VK_KHR_VIDEO_DECODE_VP9_EXTENSION_NAME);
        }
