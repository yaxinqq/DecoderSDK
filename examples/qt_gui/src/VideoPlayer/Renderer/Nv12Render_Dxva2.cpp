...
bool Nv12Render_Dxva2::interopToOpenGL(const decoder_sdk::Frame &frame)
{
    if (!frame.isValid()) {
        return false;
    }
...
bool Nv12Render_Dxva2::renderToFbo(const decoder_sdk::Frame &frame)
{
                                             ...
                                             const wgl::WglDeviceRef &wglDevice)
{
    ...
    if (wglTextureHandle_ && !unregisterWglTexture(wglD3DDevice_, wglTextureHandle_)) {
        qWarning() << QStringLiteral("[Nv12Render_Dxva2] Failed to unregister WGL object before switching device!");
        return false;
    }
...
bool Nv12Render_Dxva2::registerTextureWithOpenGL(int width, int height)
{
    ...
    if (wglTextureHandle_ && !unregisterWglTexture(wglD3DDevice_, wglTextureHandle_)) {
        qWarning() << QStringLiteral("[Nv12Render_Dxva2] Failed to unregister previous WGL object!");
        return false;
    }
