...
void VideoPlayerImpl::initialize(QOpenGLContext *context, QSurface *surface)
{
    if (!renderWorker_) {
        ...
        // connect(renderWorkerThread_, &QThread::finished, renderWorker_,
        // &RenderWorker::deleteLater);
        connect(renderWorkerThread_, &QThread::finished, renderWorkerThread_,
                &QThread::deleteLater);
        ...
        connect(this, &VideoPlayerImpl::preparePlaying, renderWorker_,
                &RenderWorker::preparePlaying);
...
void VideoPlayerImpl::paintGL(const QRect &widgetRect, const QRect &needRenderedRect,
                              const QPoint &referencePt)
{
    ...
    if (playerState_ == Stream::PlayerState::Playing ||
        ...
        curContext->functions()->glViewport(diff.x(), -diff.y(), videoRect.width(),
                                            videoRect.height());
...
void VideoPlayerImpl::paintCommon(QPainter *painter, const QRect &widgetRect)
{
    ...
    if (fpsVisible_) {
        ...
        painter->drawText(widgetRect_.topRight() - QPoint(fpsTextWidth_, -fpsTextHeight_),
...
void VideoPlayerImpl::videoFrameReady(const std::shared_ptr<decoder_sdk::Frame> &frame)
{
    ...
    if ((playerState_ == Stream::PlayerState::Start ||
         playerState_ == Stream::PlayerState::Resume) &&
        frame->isValid()) {
...
void VideoPlayerImpl::renderToImage(const QSize &size, QImage &image)
{
    ...
    if (!render_.isNull()) {
        ...
        if (sharedRender) {
            ...
            curThreadRenderWorkerContext->functions()->glViewport(0, 0, size.width(),
                                                                  size.height());
...
QRect VideoPlayerImpl::calculateVideoRect(const QRect &needRenderedRect)
{
    ...
    if (aspectRatioMode() == Stream::AspectRatioMode::KeepAspectRatio) // 保持宽高比
    {
        if ((double)width / height >=
            (double)frameWidth_ / frameHeight_) // 被横向拉伸时，以高度为基准
        {
            const int nOffset = width - height * (double)frameWidth_ / frameHeight_;
            videoRect = QRect(needRenderedRect.x() + nOffset / 2, needRenderedRect.y(),
                              width - nOffset, height);
        } else // 被纵向拉伸时，以宽度为基准
        {
            const int nOffset = height - width * (double)frameHeight_ / frameWidth_;
            videoRect = QRect(needRenderedRect.x(), needRenderedRect.y() + nOffset / 2, width,
                              height - nOffset);
        }
    } else // 忽略宽高比
    {
