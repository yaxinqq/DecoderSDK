...
RtspStreamPlayer::RtspStreamPlayer(QWidget *parent) : VideoPlayer(parent)
{
...
void RtspStreamPlayer::open(const QString &url, const QString &deviceId, const QString &channelId,
                            Stream::OpenMode openMode)
{
    ...
    const auto decoderKey = StreamManager::instance()->openStream(impl_, url, openMode);
...
{
    if ((playerState() != Stream::PlayerState::Start &&
         playerState() != Stream::PlayerState::Playing &&
